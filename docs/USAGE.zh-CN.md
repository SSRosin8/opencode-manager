...
总览页按推荐顺序提供 5 步引导：配置网关、添加节点来源、测试节点、绑定 Workers、连接 OpenCode。点击步骤会直接进入对应页面。配置完成后可以隐藏引导；顶部的问号按钮可随时重新打开使用指南。
...
进入“资源配置 → 网关”。页面顶部的请求链路用于说明代理出口应在“代理池”和“Workers”中配置；网关页面本身不负责选择 IP。

1. Base URL 保持 `https://opencode.ai/zen/v1`。
2. 建议设置网关访问令牌；客户端通过 `X-OC-Relay-Key` 传递。
3. 监听端口保持 `9876`，除非与其他服务冲突；修改端口后需要重启生效。
4. “高级兼容设置”保持折叠，CLI 身份头合成通常保持关闭；VPS、Cloudflare 或非标准客户端环境需要时再开启。
5. 点击“保存更改”。
