...
export async function handleProxyProbes(
  ...
  const { store, upstream, subscriptionFetch, probes, clashProbeQueue, freeModels, batchProbeProgress } = ctx;
...
async function finishBatchProbe(args: {
  ...
  const { res, store, upstream, probes, results, targets, batchProbeProgress, workerSyncError, incrementallyAddedWorkerIds } = args;
  if (workerSyncError) {
