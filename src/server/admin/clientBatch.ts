...
export const ADMIN_CLIENT_BATCH = `    async function loadSettings() {
    ...
    function updateBatchProgress(progress, refreshNodes = true) {
      if (!progress) return;
      ...
      if (btn) {
        ...
        btn.textContent = progress.cancelRequested
          ? t("batchCancelling")
          : progress.paused
            ? t("batchPaused")(progress.completed || 0, progress.total || 0)
            : progress.running
      ...
      const pause = $("btn-batch-pause");
      const cancel = $("btn-batch-cancel");
      if (pause) {
        pause.hidden = !progress.running || !!progress.cancelRequested;
        pause.disabled = batchControlPending || !!progress.cancelRequested;
        pause.textContent = t(progress.paused ? "resumeBatch" : "pauseBatch");
      }
      if (cancel) {
        cancel.hidden = !progress.running;
        cancel.disabled = batchControlPending || !!progress.cancelRequested;
        cancel.textContent = progress.cancelRequested ? t("batchCancelling") : t("cancelBatch");
      }
      ["btn-save-accounts", "btn-save-gateway", "btn-assign-proxies", "btn-remove-all-proxies"].forEach((id) => {
        const action = $(id);
        if (action) action.disabled = !!progress.running;
      });
      if (refreshNodes) renderNodes();
    }

    async function controlBatchTest(action) {
      if (!batchProgress?.running || batchControlPending) return;
      batchControlPending = true;
      updateBatchProgress(batchProgress, false);
      try {
        const res = await fetch("/admin/api/proxy-pool/test-batch/" + action, { method: "POST" });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error?.message || ("HTTP " + res.status));
        acceptBatchProgress(data.progress || data);
        updateBatchProgress(batchProgress, false);
        toast(action === "pause" ? t("toastBatchPaused")
          : action === "resume" ? t("toastBatchResumed") : t("toastBatchCancelling"));
        scheduleBatchProgressPoll(0);
      } catch (e) {
    ...
    function batchNodeStateChanged(previous, next) {
      if (!previous || !next || !!previous.running !== !!next.running) return true;
      const previousIds = previous.completedIds || [];
      const nextIds = next.completedIds || [];
      if (previousIds.length !== nextIds.length) return true;
      const previousSet = new Set(previousIds);
      return nextIds.some((id) => !previousSet.has(id));
    }
    ...
    async function pollBatchProgress(generation = batchPollGeneration) {
      ...
      try {
        ...
        const refreshNodes = batchNodeStateChanged(batchProgress, progress);
    ...
    async function batchTestProxies() {
      ...
      try {
        ...
        if (data.progress?.cancelled) {
          toast(t("toastBatchCancelled")(data.progress.completed || 0, data.progress.total || pool.length));
        } else {
        ...
        if (!terminalResponseHandled) await pollBatchProgress();
