...
Source: Anthropic `frontend-design` official version
(`https://github.com/anthropics/skills/tree/main/skills/frontend-design`).
The sections below are the official guidance, kept faithful. The only addition
is §0, which binds it to this repo's constraints — **§0 wins on conflict**.

## 0. opencode-manager localization (mandatory, overrides below on conflict)

- Scope: new surfaces, empty states, first-run onboarding, guide, brand
  moments. Do NOT decide Proxy/Worker information architecture, table density,
  or operational workflows with this skill alone — that belongs to
  `.opencode/skills/admin-ui-design/SKILL.md`.
- Before designing, read `AGENTS.md`,
  `.opencode/skills/dev-workflow/SKILL.md`,
  `.opencode/skills/admin-ui-design/SKILL.md`, and `tests/admin-ux-ui.test.ts`
  (do not break UX contracts).
- Preserve the `src/server/adminHtml.ts` self-contained HTML/CSS/JS assembly;
  no React/Vue/Svelte, no new frontend build, no runtime UI dependency.
- Offline local admin: system font stack (or self-hosted), no Google Fonts,
  no CDN assets.
- Content surfaces solid; translucency for chrome only (nav, menus, tooltips,
  modals). Compact radii (4-10px). Accent = selection/action only;
  green/amber/red are health-only.
- Motion restrained (tabs/popovers 120-160ms, modal 180-220ms, page switch
  instant or ≤100ms opacity); honor `prefers-reduced-motion`, visible focus,
  44px mobile targets.
- All visible copy ships EN + ZH via `clientI18n.ts`.
- Complex pages: standalone HTML mockup first, confirm, then port into
  `src/server/admin/`. Develop only in a dedicated worktree; run
  `npm run validate` + `git diff --check`; browser-check desktop + mobile,
  dark + light.
