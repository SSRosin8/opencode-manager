...
- 启动时以及之后每 15 分钟读取 OpenCode Zen 官方模型目录（`https://opencode.ai/zen/v1/models`），保留 `*-free` 模型 ID 和明确列入官方特殊免费白名单的模型；后台也可以立即手动刷新。
...
静态基线模型（运行时会从 Zen 官方模型目录刷新，以后台/API 显示为准）：
...
首次打开后台总览时会显示 5 步配置引导：配置网关、添加节点来源、测试节点、绑定 Worker、连接 OpenCode。引导步骤可以直接跳转到对应页面，也可以隐藏；顶部的问号按钮可随时重新打开完整使用指南。

后台主题使用紧凑的中性表面，导航、面板、表单、表格和主要操作采用一致的主题色层级。运行成功、警告和失败继续使用独立语义色，同时保持清晰的键盘焦点和移动端触控区域。
