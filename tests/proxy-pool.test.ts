...
describe("admin proxy-pool HTTP APIs", () => {
  ...
  it("adds manual proxy, binds worker, fetches clash subscription into pool", async () => {
    dir = await mkdtemp(join(tmpdir(), "opencode-manager-pool-"));
...
`;
  ...
  it("tests exactly one bound worker with a real proxied chat request", async () => {
    dir = await mkdtemp(join(tmpdir(), "opencode-manager-worker-test-"));
    const store = new SettingsStore(join(dir, "settings.json"));
    await store.save(
      settings({
        accounts: [
  ...
  it("returns 400 when no probe-healthy proxies exist", async () => {
    dir = await mkdtemp(join(tmpdir(), "opencode-manager-assign-empty-"));
    const store = new SettingsStore(join(dir, "settings.json"));
    await store.save(
      settings({
  ...
  it("auto-assigns unique healthy proxies to each worker", async () => {
    dir = await mkdtemp(join(tmpdir(), "opencode-manager-assign-"));
    const store = new SettingsStore(join(dir, "settings.json"));
    await store.save(
      settings({
        accounts: [
