...
describe("admin proxy probe HTTP APIs", () => {
  ...
  async function boot(
    ...
    dir = await mkdtemp(join(tmpdir(), "opencode-manager-probe-"));
  ...
  it("publishes incremental batch progress and rejects overlapping batches", async () => {
    dir = await mkdtemp(join(tmpdir(), "opencode-manager-probe-progress-"));
    const store = new SettingsStore(join(dir, "settings.json"));
    await store.save({
      baseUrl: "https://opencode.ai/zen/v1",
      accounts: [],
  ...
  it("reserves a batch before reading its body and releases invalid requests", async () => {
    dir = await mkdtemp(join(tmpdir(), "opencode-manager-probe-lock-"));
    const store = new SettingsStore(join(dir, "settings.json"));
    await store.save({
      baseUrl: "https://opencode.ai/zen/v1",
      accounts: [],
